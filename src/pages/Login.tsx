...
export default function Login() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Find user with matching credentials or fallback to demo
    const user = DUMMY_USERS.find(
      u => u.username === account && u.password === password
    ) || (account ? { id: 99, username: account, name: account, role: 'Operator' } : DUMMY_USERS[0]);
        ...
        <div className="p-6 sm:p-8">
          <form onSubmit={handleLogin} className="max-w-md mx-auto">
            ...
            <div className="mt-8 flex justify-center">
              <button
                type="submit"
                className="px-12 py-2.5 bg-gradient-to-r from-blue-200 to-blue-300 border border-blue-400 rounded-lg font-semibold text-blue-800 shadow-md hover:from-blue-300 hover:to-blue-400 transition-all text-sm sm:text-base cursor-pointer"
              >
                Login
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                {DUMMY_USERS.map((user) => (
                  <button
                    ...
                    className="px-2 py-1.5 text-xs bg-gray-50 hover:bg-blue-50 border border-gray-200 rounded-md transition-colors text-gray-600 hover:text-blue-600 cursor-pointer"
                  >
                    {user.username}
                  </button>
                ))}
